...
def RIND_attention_loss2(output,target):
...
def RIND_loss(output,label):
    batch_size, c, height, width = label.size()
    loss_focal = RIND_attention_loss2(output, label)
...
class RIND_Edge(nn.Module):

    def __init__(self):
        self.inplanes = 64
        super(RIND_Edge, self).__init__()
...
if __name__ == '__main__':
    model = RIND_Edge()
    dummy_input = torch.rand(5, 3, 320, 320)
